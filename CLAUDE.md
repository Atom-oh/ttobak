...
- **Bedrock models**: Claude Opus 4.8 for summarize/vision, Claude Haiku for fast translation/detection. Live Q&A (`backend/python/qa/handler.py`, both `/api/qa/*` HTTP and the `ask_live` WebSocket stream) uses Claude Sonnet 4.6 (`BEDROCK_MODEL_ID`) instead of Opus — conversational Q&A is latency-sensitive and Sonnet's quality is sufficient for it, at roughly 5-10x lower cost than Opus; question-detection uses Qwen3-32B (`DETECT_MODEL_ID`), not Haiku.
