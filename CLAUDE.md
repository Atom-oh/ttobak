...
### DynamoDB & S3
Table `ttobak-main`, single-table design. Key schema and GSIs in `backend/internal/model/meeting.go`. S3 keys: `{audio|images|files|transcripts}/{userId}/{meetingId}/...` (upload categories `audio`/`image`/`file` in `service/upload.go`, plus the separate `transcripts/` prefix written by the STT pipeline); account/personal document uploads (slides) use `docs/{userId}/{fileName}` — no meetingId, since a document isn't tied to a meeting. All four upload categories share one bucket-wide `bucket.grantReadWrite(apiRole)` IAM grant (`infra/lib/ai-stack.ts`), not a per-prefix policy, so adding `docs/` required no CDK change.
