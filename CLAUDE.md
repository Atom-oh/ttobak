...
- **"Both UserPoolId and ClientId are required"**: This error comes from the **frontend Cognito SDK** (`frontend/src/lib/auth.ts`), not Lambda@Edge. The frontend uses `NEXT_PUBLIC_COGNITO_USER_POOL_ID` and `NEXT_PUBLIC_COGNITO_CLIENT_ID` env vars which are embedded at `npm run build` time. In GitHub Actions, these must be set as repository variables (`gh variable set`) and injected in the `build-frontend` job. If missing during build, empty strings are baked into the static export and the Cognito SDK throws this error at runtime. **Fix**: Ensure GitHub repo variables are set and referenced in deploy.yml as `${{ vars.NEXT_PUBLIC_COGNITO_* }}`.
