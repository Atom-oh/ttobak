...
All GET download URLs returned to browsers (`downloadUrl`/`previewUrl`/`audioUrl`/attachment `url`/public-share 302) are **CloudFront-signed URLs** on the site domain (`https://{domain}/media/{s3Key}?Expires=...&Signature=...&Key-Pair-Id=...`), not raw S3 presigns — the bucket address is never exposed. One choke point: `UploadService.GeneratePresignedDownloadURLWithTTL` uses `CloudFrontSigner` (`backend/internal/service/cfsign.go`) when configured, falling back to S3 presign if the SSM key material is unreadable (local dev, partial deploy). The `/media/*` CloudFront behavior (frontend-stack.ts) pairs a trusted key group (viewer auth) with OAC (origin auth) and a prefix-strip CloudFront Function; the OAC read policy lives in StorageStack (FrontendStack imports the bucket by name — no cross-stack ref). Key material: public PEM committed at `infra/lib/cloudfront-signing-pub.pem`; private key is the manually-created SecureString `/ttobak/cloudfront/signing-key`; key-pair-id published by FrontendStack at `/ttobak/cloudfront/key-pair-id`. PUT upload presigns still use the raw S3 domain by design (background XHR only). The OAC bucket policy's `AWS:SourceArn` condition is resolved at **deploy time**, not synth time: a small inline Lambda (`MediaDistributionIdLookupFn`, behind a CDK custom resource in `storage-stack.ts`) reads FrontendStack's published SSM parameter `/ttobak/cloudfront/media-distribution-id` (same publish-and-read-by-fixed-name pattern as `key-pair-id`) and falls back to a same-account wildcard only if that parameter doesn't exist yet (first deploy, before FrontendStack has ever run) — StorageStack can't hold a direct CDK reference to a distribution ARN that FrontendStack (deployed later, depends on Gateway+EdgeAuth+Auth) hasn't created. This fallback is a **ratchet**, not a plain cache: every time the Lambda sees a real ID it also writes it to its own state parameter (`/ttobak/cloudfront/media-distribution-id-last-known-good`), and checks that state parameter before falling back to the wildcard — so a policy that's already been tightened can't silently widen itself back open if the source parameter is later deleted or renamed; only a genuinely-first deploy (neither parameter has ever existed) uses the wildcard. No manual step or `cdk.json` edit is needed: the custom resource's `Timestamp` property changes on every synth, forcing CloudFormation to re-invoke the Lambda (not skip it as a no-op) on every `TtobakStorageStack` deploy — including `deploy-infra.yml`'s every-push `--exclusively` redeploy — so once FrontendStack has published the real ID, every subsequent deploy self-tightens automatically. The warm-Lambda lazy retry (`cfSignerRetryAt` in `upload.go`) only fires when `cfSigner == nil` — rotating the signing key (new key → SSM update → remove old key from the CloudFront key group) still requires restarting/redeploying the `api` Lambda to pick up the new key material; the retry path exists for the initial-setup race, not for rotation.
...
- ~~**Single audio file per meeting**~~ — **FIXED** (ADR-014 Phases 1–6 are all implemented). Multi-file audio lives in `Meeting.AudioKeys`/`AudioPartCount`/`AudioPartsReady`, read through `GetEffectiveAudioKeys()` which falls back to `[AudioKey]` for pre-existing single-file meetings; parts upload as `part_{NNN}_{filename}`, transcribe per-part to `transcripts/{meetingId}_part_{NNN}.json`, and `cmd/summarize`'s `handlePartTranscript` counts them in (via `IncrementAudioPartsReady` — the counter lives in summarize, not transcribe) before emitting the custom `AllPartsTranscribed` EventBridge event that triggers `mergePartTranscripts`. `Meeting.AudioKey` survives only as that backward-compat fallback. Linked follow-up meetings are live too: `Meeting.LinkedMeetingIDs` (max 3), `POST /api/meetings/{meetingId}/link`, the `LinkMeetingsModal` picker off `MeetingHeader`, and `cmd/summarize/main.go`'s predecessor-summary context injection.
...
- **NEVER set `selfSignUpEnabled: true` on the Cognito User Pool** — company security policy: accounts are created by an admin only, via `MeetingService.InviteUser` → `AdminCreateUser` (`POST /api/settings/invite-user`, admin-gated). `infra/lib/auth-stack.ts` declares `selfSignUpEnabled: false` explicitly (synthesizing `AllowAdminCreateUserOnly: true`); don't "fix" it back to true. The `ttobak-pre-signup` email-domain allowlist Lambda (ADR-007) is defense in depth only — it fires solely on the now-closed self sign-up path, so it is NOT the control that restricts who can join. Consequence for the frontend: there is **no sign-up form** (removed — it only produced a confusing Cognito "sign-up is not enabled" error). An invited user logs in with the temporary password from the invite email and completes the `NEW_PASSWORD_REQUIRED` challenge in `LoginForm`; `lib/auth.ts` no longer exports `signUp`/`confirmSignUp` at all.
