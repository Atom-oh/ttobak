...
- iOS "녹음 복구" always failing — the checkpoint recovery lookup was hardcoded to `.webm`, but iOS Safari's MediaRecorder only produces `.m4a`; now located by prefix instead of a fixed extension
...
- iOS "녹음 복구"가 항상 실패하던 문제 — 체크포인트 조회가 `.webm`으로 하드코딩돼 있었는데 iOS Safari의 MediaRecorder는 `.m4a`만 생성함; 고정 확장자 대신 접두사로 찾도록 수정
