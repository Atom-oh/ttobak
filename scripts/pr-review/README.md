...
Claude and Kiro share their existing `PANEL_TIMEOUT × PANEL_RETRIES` invocation-time
budget across attempts instead of restarting slow responses at each nominal timeout.
...
and at most two calls. Codex keeps its per-attempt limit. Kiro startup probes keep
their separate existing limits; the shared review deadline starts afterward.
Terminal failures,
