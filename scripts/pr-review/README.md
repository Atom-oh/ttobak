...
This change selects `ROLE_REVIEW=1` in CI. The workflow determines which protocol
runs; the legacy entrypoints remain available as the documented compatibility path.
Trusted inputs feed specialist executors, validated results feed aggregation, and
substantive candidates go to the chair. See the
[project contract](../../docs/pr-review-specialists.md).
...
Tags are protocol identifiers; `kiro-fable` requests Opus. Kiro aliases and Bedrock
Runtime IDs are different namespaces. Sol replaces the legacy Terra review slot;
application inference models remain unchanged. Configured IDs do not attest weights.
...
aggregation rechecks the anchor and reports excluded paths. The collector does not select the workflow mode; CI supplies `ROLE_REVIEW=1`.
