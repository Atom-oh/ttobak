'use client';

import { useState } from 'react';

interface RecordingTabsProps {
  captionsContent: React.ReactNode;
  translationContent: React.ReactNode;
  summaryContent: React.ReactNode;
  notesContent: React.ReactNode;
}

type TabId = 'summary' | 'notes' | 'captions' | 'translation';

const tabs: { id: TabId; label: string; icon: string }[] = [
  { id: 'summary', label: '요약', icon: 'summarize' },
  { id: 'notes', label: '노트', icon: 'edit_note' },
  { id: 'captions', label: '자막', icon: 'subtitles' },
  { id: 'translation', label: '번역', icon: 'translate' },
];

export function RecordingTabs({ captionsContent, translationContent, summaryContent, notesContent }: RecordingTabsProps) {
  const [activeTab, setActiveTab] = useState<TabId>('summary');

  return (
    <div className="flex flex-col">
      {/* Tab Bar */}
      <div className="flex border-b border-slate-200 dark:border-white/10 mb-4">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium transition-colors relative ${
              activeTab === tab.id
                ? 'text-primary'
                : 'text-slate-500 dark:text-text-muted hover:text-slate-700 dark:hover:text-text-secondary'
            }`}
          >
            <span className="material-symbols-outlined text-[18px]">{tab.icon}</span>
            {tab.label}
            {activeTab === tab.id && (
              <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-full" />
            )}
          </button>
        ))}
      </div>

      {/* Tab Content - all rendered, visibility toggled */}
      <div className={activeTab === 'summary' ? '' : 'hidden'}>{summaryContent}</div>
      <div className={activeTab === 'notes' ? '' : 'hidden'}>{notesContent}</div>
      <div className={activeTab === 'captions' ? '' : 'hidden'}>{captionsContent}</div>
      <div className={activeTab === 'translation' ? '' : 'hidden'}>{translationContent}</div>
    </div>
  );
}
