...
export function Table(props: TableHTMLAttributes<HTMLTableElement> & { children?: ReactNode }) {
  return (
    <div className="my-4 overflow-x-auto rounded-xl border border-slate-200 dark:border-white/[0.06]">
...
export function TBody(props: { children?: ReactNode }) {
  return <tbody className="[&>tr:nth-child(even)]:bg-slate-50 dark:[&>tr:nth-child(even)]:bg-white/[0.02]">{props.children}</tbody>;
}

export function TR(props: { children?: ReactNode }) {
  return <tr className="border-b border-slate-200 dark:border-white/[0.05] last:border-none">{props.children}</tr>;
}
