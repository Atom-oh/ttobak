 ...
 *
 * `ceiling` is intentionally NOT floored at `min` -- when the container
 * truly can't fit `min` (e.g. a sibling column also claiming space), the
 * only options are shrink below the stated floor or overflow the row, and
 * shrinking is strictly better. The caller's own CSS should still allow
 * that shrink (no `shrink-0` paired with `reserve`) as a last-resort net
 * beyond this hook's own math.
 */
export function useResizablePanel(
  ...
  const width = clamp(preferred, min, ceiling);
