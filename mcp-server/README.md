...
| `ttobak_kb_sync` | Trigger Knowledge Base ingestion for uploaded files (returns "skipped" on a deployment missing the KB env vars this PR adds — see tool description) | "Sync the KB now" |
...
| `ttobak_kb_sync` | 업로드된 파일에 대해 Knowledge Base 인제스천 실행 (이 PR이 추가한 KB env가 없는 배포에서는 "skipped" 반환 — 도구 설명 참조) | "지금 KB 동기화해줘" |
