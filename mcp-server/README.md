...
| `ttobak_update_project` | Update a project (whole-record replace -- resend name even if unchanged) | "Rename that project to Hana Renewal 2.0" |
...
| `ttobak_update_project` | 프로젝트 수정(전체 레코드 교체 — 변경 없어도 name 재전송 필요) | "그 프로젝트 이름을 하나은행 갱신 2.0으로 바꿔줘" |
