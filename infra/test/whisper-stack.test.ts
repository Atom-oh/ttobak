...
describe('WhisperStack legacy task definition (ADR-035)', () => {
  test('does NOT set DIARIZATION_S3_KEY, and keeps its other required env', () => {
    ...
    // resurrect the mismatch window ADR-035 closed. The presence checks pin
    // the rest of the env so this test can't pass vacuously against a
    // container whose environment went missing entirely.
    const template = synth();
    const taskDefs = template.findResources('AWS::ECS::TaskDefinition', {
    ...
    for (const container of defs[0].Properties.ContainerDefinitions) {
      ...
      expect(names).toEqual(
        expect.arrayContaining(['BUCKET_NAME', 'TABLE_NAME', 'MODEL_S3_KEY', 'VOCAB_KEY']),
      );
    }
  });
});
