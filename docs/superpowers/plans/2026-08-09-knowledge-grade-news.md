...
Use the existing `useTheme()` hook to observe theme changes; re-render Mermaid
when its theme value changes. Do not add another component-local
`MutationObserver`. Keep `securityLevel: 'strict'`.
...
1. pause existing news schedules and verify they remain disabled throughout deployment, canary, and migration;
2. deploy `TtobakAiStack --exclusively`;
3. deploy `TtobakCrawlerStack --exclusively` with scheduled news execution disabled;
4. deploy `TtobakGatewayStack --exclusively`;
5. deploy `TtobakFrontendStack --exclusively` for runtime config and infrastructure;
6. publish the frontend build through the existing frontend deployment workflow (S3 sync must preserve `config.json`, followed by CloudFront invalidation);
7. run one source canary;
8. verify full-fetch, quality, cost, and no-publication alarms;
9. run migration dry-run;
10. migrate bounded source batches;
11. verify KB ingestion, API visibility, and the deployed UI;
...
Edit `CLAUDE.md` with new files, model variables, test commands, and deployment
order. Do not hand-edit `AGENTS.md`: regenerate it with the repository context-sync
command, then stage the generated result in Step 7. If the sync tool is unavailable,
leave `AGENTS.md` unchanged and report that its generated context remains stale.
