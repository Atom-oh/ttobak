...
- **TTL**: `timeToLiveAttribute: 'pendingShareExpiresAt'` -- scoped to the attribute `PendingShare` writes (`backend/internal/model.PendingShare`'s `TTL` Go field, tagged `dynamodbav:"pendingShareExpiresAt"`), deliberately NOT the uppercase `TTL` attribute `backend/python/qa/handler.py` already writes on rate-limit/KB-cache/conversation-MESSAGES/CHAT_SESSION rows. A table has only one `timeToLiveAttribute`, but DynamoDB's sweep only deletes items that actually carry that exact attribute name, so pointing it at PendingShare's own distinctly-named attribute lets it auto-clean expired pending invites without also bulk-deleting QA's pre-existing, never-swept conversation history -- that separate cleanup remains a deliberate decision for its own PR. One other row family deliberately opts INTO the sweep by writing this same attribute: the QA web-search hourly rate-limit counter (`check_web_search_limit`, `USER#{id}/WEBSEARCH_HOURLY#{hour}`, 2h expiry) -- unlike `check_research_limit`'s rows (uppercase `TTL`, never swept), these would otherwise accumulate one row per user per hour forever. `MeetingService.MaterializePendingShares` still enforces the same expiry synchronously in application code (an expired row fails that check and is dropped on the spot the next time that exact email authenticates), since DynamoDB's own sweep can lag real time by up to 48 hours -- the table-level TTL is a physical-reclaim guarantee for rows nothing ever touches again (e.g. a mis-typed email, or an invite the inviter forgets to revoke), not the correctness gate.
...
#### KB Lambda
- Trigger: one-minute scheduled tick (CDK deployment configuration:
  `knowledgeIndexScheduleEnabled=true`, `knowledgeIndexingMode=all`),
...
- Permissions: scoped `grantRead('docs/*')` + `grantRead('docs-pdf/*')` + `grantPut('docs-pdf/*')` — narrower than the bucket-wide `grantReadWrite` other upload-triggered Lambdas share, since this role runs a third-party parser (LibreOffice) against untrusted content. The `soffice` subprocess has every `AWS_*` env var stripped before exec. Preview HEAD reads support conditional replacement; source ETag/version metadata binds the generated bytes. Reads remain cross-tenant within `docs/*` and `docs-pdf/*` (any user's originals/previews, not just the triggering key) — tracked as a residual risk in ADR-022, not yet closed.
...
- **Network mode**: HOST
- **Container**: `whisper` — image `ttobak-whisper:latest` (ECR); memory 12,288 MiB (reserves 4GB for OS/ECS agent); GPU: 1; env `BUCKET_NAME`, `TABLE_NAME`, `AWS_REGION`, `VOCAB_KEY`, `MODEL_S3_KEY` — deliberately NOT `DIARIZATION_S3_KEY` since Phase 2 (2026-09-03, ADR-035): the diarization bundle key's source of truth is `transcribe.py`'s in-image default (`models/whisperx-diarization-4.x.tar.gz`, the pyannote 4.x community-1 bundle, downloaded once per task to `/tmp/diarization-model`; the key name is bench-lineage, the content engine-neutral), so the bundle generation and the image's pyannote pin ship ATOMICALLY via deploy-whisper.yml — a CDK-set key would ship via deploy-infra.yml and the two workflows race on merge pushes, leaving a mismatched pairing that degrades diarization to a fallback (loud+skipped in the new-image direction via `_bundle_pyannote_mismatch`, silent in the old-image direction). A `whisper-stack.test.ts` assertion guards against re-adding the env var. ROLLBACK: revert the whole ADR-035 commit — a partial PIN revert fails the image's `verify_pins.py` build gate, and a key-only revert builds but loud-skips diarization at runtime (`_bundle_pyannote_mismatch`) — then run BOTH deploy-whisper.yml (rebuild: pins + in-image 3.1 default) AND deploy-infra.yml (the revert resurrects the 3.1 env line in `whisper-stack.ts`); the 3.1 bundle stays in S3, and per-run experiments may still override the key via RunTask env. deploy-whisper.yml's task-def clone step also strips any stale `DIARIZATION_S3_KEY` from the environment it copies, so a pre-ADR-035 env can't outlive the CDK deploy that removed it. See ADR-035; ADR-019 for the unchanged surrounding pipeline; logging: CloudWatch (`whisper` prefix)
...
A second ECR repo (`ttobak-whisperx`) and ECS task definition (family `ttobak-whisperx`, container `whisperx`) sit alongside the production `ttobak-whisper` resources above, sharing the same cluster/ASG/capacity provider (not splitting the GPU pool) but with its own IAM task role — `ttobak-whisperx-task-role`, kept separate from the legacy `ttobak-whisper-task-role` because reusing that role would give every bench run full bucket read/write + table read/write, far more than a benchmark harness needs. The execution role (`ttobak-whisper-execution-role`, ECR pull + log delivery only) stays shared — that's not a data-plane privilege. `ttobak-whisperx-task-role`'s grants are scoped to the benchmark's actual needs: read on `audio/*`, `models/*`, `config/*`; write on ONLY `bench-transcripts/*` (its own output prefix) — there is no `transcripts/*` write grant, since Phase 1 never legitimately writes there. `validate_output_key` in `transcribe_whisperx.py` still ACCEPTS the calling meeting's own `transcripts/{meetingId}[...].json` key as a deliberate Phase-2 drop-in escape hatch at the application layer, but with no matching IAM grant, pointing `OUTPUT_KEY` at it in Phase 1 now also fails at the IAM layer (S3 `PutObject` `AccessDenied`) — correct defense-in-depth on top of the application-level gate, not a gap. (The `fw_p4` hybrid engine goes further: `transcribe_fw_p4.py`'s own validator refuses the escape hatch outright — it is bench-only by construction.) Phase 2 (ADR-035) resolved AGAINST cutting over to this task definition — production keeps the legacy engine and only its diarization model changed — so the escape hatch stays dormant; a future cutover would need the `transcripts/*` write grant added back (or the legacy role reused) for it to actually work. Deliberately **no DynamoDB grant** — `mark_meeting_error` (`whisper_common.py`) is best-effort and just logs the `AccessDenied` when a bench run's error path tries to write meeting status; a future cutover to this task definition (rejected by ADR-035 for now) would need to either grant table read/write or reuse the legacy role. Note: `TABLE_NAME` is still a required env var at container import time (`transcribe_whisperx.py` reads `os.environ["TABLE_NAME"]` at module load) even though the role carries no DynamoDB grant — `mark_meeting_error` fails `AccessDenied` harmlessly on that best-effort path, logged but never raised; omitting the env var entirely would instead crash at import with a `KeyError`, before any benchmark work starts, so it stays required despite the role granting it nothing. Container logs go to a dedicated log group `/ttobak/whisperx` (30-day retention, `RemovalPolicy.DESTROY`) rather than the shared default — `aws/*` is reserved by AWS, so custom log groups use the `/ttobak/*` convention. Container env adds `WHISPERX_DIARIZATION_S3_KEY` (default `models/whisperx-diarization-4.x.tar.gz`) and `WHISPERX_BATCH_SIZE` (default `8`, conservative vs. whisperx's own default of 16 to bound peak VRAM on the shared 24GB A10G). The per-run `ENGINE` selection knob read by `run_engine.py` is deliberately NOT set in the task definition — it is supplied via RunTask environment overrides only. Reuses the same staged `MODEL_S3_KEY` (CT2 large-v3 weights are engine-compatible). Outputs: `WhisperXTaskDefArn` (export `TtobakWhisperXTaskDefArn`), `WhisperXEcrRepoUri` (export `TtobakWhisperXEcrUri`). This task definition is never invoked automatically by any pipeline — it existed to benchmark pyannote 4.x against the then-production 3.1 engine (ADR-019 follow-up; production itself runs pyannote 4.x community-1 since ADR-035) and remains available for future engine comparisons and is run manually by operators; see `docs/runbooks/whisperx-benchmark.md`. The image's ENTRYPOINT is pinned to `run_engine.py`, an allowlisting dispatcher: the engine (`whisperx` default, or the `fw_p4` legacy-ASR + community-1-diarization hybrid) is selected ONLY via the `ENGINE` env var, and any `containerOverrides` command is loudly rejected. Never set `entryPoint`/`command` on this task definition in CDK — a CDK `entryPoint` would silently bypass the dispatcher pin on a host-networkMode task whose role reads every user's audio, and CDK is the only bypass channel (ECS RunTask `containerOverrides` has no `entryPoint` field; a `command` override is loudly rejected by the dispatcher); a `whisper-stack.test.ts` assertion (run by test-infra.yml at PR time and deploy-infra.yml before deploys) guards the CDK half, `backend/whisper/test_dockerfile_entrypoint.py` (test-backend.yml) guards the image half (dispatcher ENTRYPOINT present, no CMD), and a CLAUDE.md gotcha records the invariant. **`OUTPUT_KEY` contract diverges from the legacy task**: `transcribe_whisperx.py` requires an explicit `OUTPUT_KEY` and fails fast (raises before any download/GPU work) if it's unset or blank, whereas the legacy `transcribe.py` defaults to `transcripts/{meetingId}.json` when `OUTPUT_KEY` is omitted. Benchmark run output lives under S3 prefix `bench-transcripts/`, deliberately outside both the `storage-stack.ts` OAC read allowlist and the `transcripts/`-prefixed EventBridge rule that triggers `ttobak-summarize` — so a bench artifact is never served as a download and never fires the summarize pipeline.
