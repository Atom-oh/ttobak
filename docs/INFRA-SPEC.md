...
- **Network mode**: HOST
- **Container**: `whisper` — image `ttobak-whisper:latest` (ECR); memory 12,288 MiB (reserves 4GB for OS/ECS agent); GPU: 1; env `BUCKET_NAME`, `TABLE_NAME`, `AWS_REGION`, `VOCAB_KEY`, `MODEL_S3_KEY`, `DIARIZATION_S3_KEY` (default `models/whisperx-diarization-4.x.tar.gz` since Phase 2, 2026-09-03 — the pyannote 4.x community-1 bundle, downloaded once per task and unpacked to `/tmp/diarization-model`; the key name is bench-lineage, the content is engine-neutral. ROLLBACK is two-sided: the 3.1 bundle stays in S3, but its key is CDK-hardcoded in `whisper-stack.ts` (stack redeploy, not a live env edit) and must be reverted TOGETHER with the image's pyannote/torch pins — a 4.0.7 image on the 3.1 bundle, or the reverse, fails config-incompatible and degrades to a SILENT unlabeled fallback. The same mismatch exists during deploys: the image ships via deploy-whisper.yml and the task-def env via deploy-infra.yml, so after merging a diarization change run BOTH workflows back-to-back and confirm the community-1 bundle exists in the bucket first; until both sides land, diarization is silently disabled. See ADR-035; ADR-019 for the unchanged surrounding pipeline); logging: CloudWatch (`whisper` prefix)
