...
**어카운트** 레이블의 단일 선택 `<select>`를 표시한다. 기본값은 **전체
어카운트**이며, `accountApi.list()`의 접근 가능한 어카운트를 이름 기준
한국어 로케일로 정렬한다. 컨트롤은 모바일에서 여러 줄로 감싸며 기존
입력창의 테두리, 배경, 포커스 색상을 사용한다.

- 어카운트 선택은 서버 필터(`GET /api/meetings?accountId=…`)이며 전체
  선택 시 `accountId`를 생략한다. 기존 태그(OR), 검색, Recent의 최근 7일
  필터는 받은 목록에 함께 적용한다. 탭과 선택한 어카운트는 부모가 관리한다.
