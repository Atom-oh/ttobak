...
```
브라우저 녹음 → S3 업로드 → EventBridge → Transcribe Lambda → S3 transcript
...
4. **A/B STT 엔진**: AWS Transcribe와 Nova Sonic을 동시 실행하여 비교.
   - *이유*: 한국어 STT 품질 벤치마킹, 사용자가 선호 엔진 선택 가능
...
| **Processing** | STT Pipeline | AWS Transcribe, Nova Sonic, Web Speech API |
...
```
Browser Recording → S3 Upload → EventBridge → Transcribe Lambda → S3 Transcript
