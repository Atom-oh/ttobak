...
- [ADR-032: 요약 파이프라인 타임아웃 복원력 — refine 병렬화 + 정체 시도 재처리](decisions/ADR-032-summarize-timeout-resilience.md) (승인됨)
...
- [ADR-032: Summarize Pipeline Timeout Resilience — Refine Parallelization + Stale-Attempt Reprocessing](decisions/ADR-032-summarize-timeout-resilience.md) (Accepted)
