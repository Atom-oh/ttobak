...
### Risks
- `UpdateMeeting`'s whole-item `PutItem` pattern remains unfixed for every field other than `ProjectIDs` (e.g. `AccountID`, `SharedToAccount`) — this ADR closes the race for the one field it introduces, not the underlying pattern. A broader fix (switching `UpdateMeeting` to a partial `UpdateItem`, or auditing every call site) is a pre-existing, cross-cutting issue tracked as a follow-up, not in this ADR's scope.
- `MeetingProjectLinkTransactional`/`ResearchProjectLinkTransactional` condition-check the target meeting/research but not the project CONFIG item's existence — if the project is deleted in the narrow window after `requireProjectAccess` succeeds but before the transaction commits, the link succeeds against a project that no longer canonically exists (an orphan ref pointing at a ghost `projectId`, invisible via fail-closed reads on the project side, but still holding a `projectId` string in the meeting's/research's `ProjectIDs` set). `ProjectAccountLinkTransactional` does not have this gap (its condition-check targets the project CONFIG item directly). Low-probability, low-impact (no data ever becomes visible incorrectly), tracked as a follow-up rather than fixed here.
...
### 위험
- `UpdateMeeting`의 whole-item `PutItem` 패턴은 `ProjectIDs` 외 다른 필드(`AccountID`, `SharedToAccount` 등)에 대해서는 여전히 고쳐지지 않았다 — 이 ADR은 이 기능이 도입한 필드 하나의 race만 닫았을 뿐, 근본 패턴을 고치지 않았다. 더 넓은 범위의 수정(`UpdateMeeting`을 부분 `UpdateItem`으로 바꾸거나 모든 호출부를 감사하는 것)은 기존부터 있던 범-엔티티 이슈로 이 ADR 스코프 밖에서 후속 트래킹한다.
- `MeetingProjectLinkTransactional`/`ResearchProjectLinkTransactional`은 대상 미팅/리서치는 condition-check하지만 project CONFIG 아이템의 존재는 확인하지 않는다 — `requireProjectAccess` 성공 이후 트랜잭션 커밋 이전의 좁은 창에서 프로젝트가 삭제되면, canonical하게는 더 이상 존재하지 않는 프로젝트에 대해 링크가 성공한다(프로젝트 쪽 fail-closed 읽기로는 보이지 않는 ghost `projectId`를 가리키는 고아 ref, 그리고 미팅/리서치의 `ProjectIDs` 집합에 남는 문자열). `ProjectAccountLinkTransactional`은 이 간극이 없다(project CONFIG 아이템을 직접 condition-check함). 발생 확률과 영향 모두 낮음(잘못된 데이터가 노출되는 경우는 없음)이며, 여기서 고치지 않고 후속 작업으로 트래킹한다.
