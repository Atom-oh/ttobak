...
### Risks
- ~~Removing a member from an account does not revoke previously granted per-meeting shares~~ — **superseded**: `RemoveMember` attempts a best-effort revocation of every `origin=="account"` Share for the removed member across all of the account's `MeetingRef`s, surfacing any per-meeting failure in the response (`cleanupFailedForMeetings`) instead of only in logs. But that revocation attempt is not what actually enforces access anymore: every read path that can see an `origin=="account"` Share row (meeting detail's `checkAccess`, the meeting list's `ListMeetings`, KB Q&A's `KnowledgeService.Ask`, and the Python Q&A Lambda's `_list_shared_meetings`, in `meeting.go`/`knowledge.go`/`backend/python/qa/handler.py`) re-verifies live account membership at READ TIME instead of trusting the row unconditionally. In the Python path, `SHARED_MEETINGS_CACHE_TTL_SECONDS` (default 300s) only caches the immutable identity (meetingId/ownerId) of which meetings are shared -- whether each share still exists, its current origin, the meeting's `sharedToAccount`, and live membership are all re-fetched on every call, uncached, so revocation is visible on the very next QA request regardless of that TTL. The KB-retrieval-result cache (`KB_CACHE_TTL_SECONDS`, default 600s) stores an access signature derived from that same live check alongside each cached result and re-derives it on every read, so a cache hit whose access has since changed (revoked, un-shared) is treated as a miss and re-fetched live -- it does not bypass the membership check. This means a removed member loses access essentially immediately even if the cleanup delete never ran (transient DynamoDB error, or a `ShareMeetingToAccount` write racing the `RemoveMember` snapshot) — the best-effort cleanup is now a housekeeping optimization (avoids an extra membership lookup on every future read of an already-fully-cleaned-up share) rather than the sole enforcement mechanism, closing the permanent-access gap without needing a reconciliation job. New account-origin shares are created transactionally (`CreateShareIfMember`, closing the `AddMember`↔`ShareMeetingToAccount` TOCTOU gap). This re-verification does NOT apply to pre-existing shares written before the `Origin` field existed (`origin==""` collapses indistinguishably with a direct share, so it's trusted unconditionally like any other direct grant) — see `docs/API-SPEC.md`'s Known limitation note; `backend/cmd/backfill-share-origin` is a manually-run, one-account-at-a-time remediation CLI that tags them for accounts an operator has reviewed.
...
### 위험
- ~~Account에서 멤버 제거 시 이미 부여된 미팅별 share는 회수되지 않음~~ — **대체됨**: `RemoveMember`가 제거된 멤버의 `origin=="account"` Share를 해당 Account의 모든 `MeetingRef`에 걸쳐 best-effort로 회수 시도하며, 미팅별 실패는 응답(`cleanupFailedForMeetings`)으로 노출된다. 하지만 실제 접근 통제는 이 회수 시도가 아니다 — `origin=="account"` Share row를 볼 수 있는 모든 read path(미팅 상세의 `checkAccess`, 미팅 목록의 `ListMeetings`, KB Q&A의 `KnowledgeService.Ask`, Python Q&A Lambda의 `_list_shared_meetings` — `meeting.go`/`knowledge.go`/`backend/python/qa/handler.py`)가 그 row를 무조건 신뢰하지 않고 현재 account membership을 읽기 시점에 즉시 재검증한다. Python 경로에서 `SHARED_MEETINGS_CACHE_TTL_SECONDS`(기본 300초) 동안 캐싱되는 것은 어느 미팅이 공유돼 있는지의 불변 식별자(meetingId/ownerId)뿐이며, 각 share의 존재 여부·현재 origin·미팅의 `sharedToAccount`·membership은 매 호출마다 캐시 없이 재조회하므로, 제거는 그 TTL과 무관하게 바로 다음 QA 요청부터 반영된다. KB 검색결과 캐시(`KB_CACHE_TTL_SECONDS`, 기본 600초)도 이 live 재검증으로 만든 접근 서명을 결과와 함께 저장하고 매 읽기 시점 재도출해 대조하므로, 캐시된 뒤 접근이 바뀌면(제거, un-share 등) 같은 질문이라도 캐시 미스로 처리되어 membership 검사를 건너뛰지 않는다. 따라서 cleanup의 삭제 자체가 실패해도(일시적 DynamoDB 오류, 또는 `RemoveMember`의 snapshot과 경쟁하는 `ShareMeetingToAccount` 쓰기 등) 제거된 멤버는 거의 즉시 접근을 잃는다 — best-effort cleanup은 이제 유일한 집행 수단이 아니라 하우스키핑 최적화(이미 완전히 정리된 share에 대해 매 읽기마다 추가 membership 조회를 피함)일 뿐이며, reconciliation job 없이도 영구 접근 잔존 갭이 닫혔다. 신규 account-origin share는 트랜잭션(`CreateShareIfMember`)으로 생성돼 `AddMember`↔`ShareMeetingToAccount` 사이 TOCTOU 갭도 닫혔다. 이 재검증은 `Origin` 필드 도입 이전에 쓰인 기존 share에는 적용되지 않는다(`origin==""`은 direct share와 구분 불가능하게 겹쳐 다른 direct grant와 동일하게 무조건 신뢰됨) — `docs/API-SPEC.md`의 Known limitation 참고; `backend/cmd/backfill-share-origin`이 운영자가 검토한 계정을 한 번에 하나씩 태깅하는 수동 실행 remediation CLI다.
