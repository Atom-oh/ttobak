...
- A recording is no longer at risk of being silently lost: the WAV is deleted only after a confirmed upload, uploads survive being large and slow, and retries resume instead of restarting from scratch.
...
- 녹음이 조용히 사라질 위험이 없어짐: WAV는 업로드가 확인된 뒤에만 삭제되고, 업로드는 크고 느려도 견디며, 재시도는 처음부터 다시 시작하지 않고 이어서 진행됨.
