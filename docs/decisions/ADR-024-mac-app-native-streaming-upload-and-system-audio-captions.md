...
- **Retry is network-aware, not just count-bounded.** `uploadRecordingWithRetry` waits for the browser's `online` event when offline (capped at 30 minutes — comfortably inside the presigned URL's 1h TTL) rather than failing immediately, and **re-presigns before every attempt** rather than reusing the URL from the first `getPresignedUrl` call — the offline wait alone could otherwise outlast the URL's TTL, turning "waited, then retried" into "waited, then failed anyway with a stale URL." Once online, a non-network failure (bad URL, server error) gets a small bounded retry (2 attempts) instead of waiting again. The wait is cancellable via `AbortController`, aborted by `usePostRecording`'s `reset()`/`createDraftMeeting()` so an abandoned or superseded flow never resumes a stale upload later.
- **One version-skew message, one place.** `VERSION_SKEW_MESSAGE` (`lib/tauri.ts`) is shared by the preflight failure and the post-recording upload catch (`usePostRecording.ts`'s `isCommandNotFound` branch) instead of two independently-worded strings.
