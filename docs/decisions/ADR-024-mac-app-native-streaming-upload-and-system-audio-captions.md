...
- The remote capability is restricted to the application origin. Native capture has no Web Speech fallback; caption failure must not discard the recording. (Superseded in part by [ADR-046](ADR-046-mac-core-audio-tap-and-microphone.md): capture now uses a Core Audio tap and includes the microphone.)
- `begin_start` reserves the recorder through an RAII `StartGuard`. Start/stop release the recorder lock before blocking capture FFI calls. Stop removes the handle and inserts its canonical path into the finalizing set in the **same critical section**.
