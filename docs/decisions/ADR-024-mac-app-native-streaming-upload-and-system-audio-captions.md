...
- **Preflight contract.** `assertUploadRecordingAvailable` (`frontend/src/lib/tauri.ts`) invokes `upload_recording` with an empty path *before* native capture starts, relying on an implicit contract with the Rust side: if the command exists, `validate_recording_path`'s `canonicalize("")` fails with `AppError::Io` before any FS/network work, so any rejection other than "command not found" means the command IS present. This is a probe, not a validated API — if the Rust-side validation order ever changes, this degrades silently rather than breaking (a genuinely missing command is still caught; other cases just pass through, logged via `console.debug` for visibility).
- **Retry is network-aware, not just count-bounded.** `uploadRecordingWithRetry` waits for the browser's `online` event when offline rather than failing immediately, and **re-presigns before every attempt** rather than reusing the URL from the first `getPresignedUrl` call — the offline wait alone could otherwise outlast the presigned URL's 1h TTL (`upload.go`'s `GeneratePresignedUploadURL`), turning "waited, then retried" into "waited, then failed anyway with a stale URL." The offline wait is bounded by a *cumulative* 45-minute wall-clock deadline for the whole call, not a fresh timer per offline cycle — a flapping connection (offline→online→offline) still eventually gives up rather than resetting its budget every cycle — and each re-entry into the wait backs off briefly instead of tight-looping the presign endpoint. Once online, a non-network failure (bad URL, server error) gets a small bounded retry (2 retries beyond the first attempt) instead of waiting again.
- **Abort alone can't stop an in-flight flow — a generation counter closes the gap.** `AbortController` only cancels the *offline wait itself*; it can't retroactively cancel a `upload_recording` PUT already past that point, or an awaited `meetingsApi` call earlier in the flow. `usePostRecording`'s `flowGenerationRef` — bumped by `reset()`/`createDraftMeeting()` — is checked after every await in `resumeUploadFlow` (including in its `catch`), so a flow the user already walked away from (Home, or starting a new recording) can neither resurrect an error banner nor complete `notifyComplete`/`cleanupRecording`/redirect for a recording that's no longer the active one. A `useEffect` unmount cleanup aborts any still-pending wait as a backstop for navigating away without going through `reset()`.
