...
The following enhancements were added after the initial implementation:

1. **Tag-based filtering**: `ListInsights` API and Insights UI now support multi-tag filtering (`tags []string` parameter). Documents can be filtered by AWS service tags in addition to source and type filters.
2. **Inline article reader**: Articles can be read directly within the Insights page without navigating away. Full content is fetched from S3 and rendered with markdown support (remark-gfm, rehype-raw).
3. **Article detail page**: `/insights/{sourceId}/{docHash}` provides a dedicated view for each crawled document with full content, metadata, and related articles.
4. **Research tab**: The Insights page gained a third tab (News | Tech | Research) integrating with the Deep Research Agent (see separate ResearchAgentStack). Users can launch multi-source research jobs and view completed reports inline.
5. **Claude Sonnet 4.6 summaries**: Crawler summarization upgraded from Haiku to Sonnet 4.6 for higher-quality document summaries.
...
- Tag filtering enables focused browsing by AWS service (e.g., show only EKS-related articles)
- Inline article reader reduces context switching when reviewing crawled content
- Research tab provides deep-dive analysis capability alongside curated news/tech feeds
- Estimated cost is low (~$12-27/month) due to serverless architecture
...
- Sonnet 4.6 summarization costs more per document than Haiku, though total volume remains low
...
- `infra/lib/research-agent-stack.ts` -- Deep Research Agent (Bedrock Agent + tool Lambdas)
...
- `backend/internal/service/insights.go` -- ListInsights with tag/service/source filtering
- `frontend/src/app/insights/` -- Insights page with News, Tech, and Research tabs
...
초기 구현 이후 다음 기능이 추가되었습니다:

1. **태그 기반 필터링**: `ListInsights` API와 Insights UI가 다중 태그 필터링(`tags []string` 파라미터)을 지원합니다. 소스 및 타입 필터 외에 AWS 서비스 태그로 문서를 필터링할 수 있습니다.
2. **인라인 기사 리더**: Insights 페이지 내에서 페이지 이동 없이 기사를 직접 읽을 수 있습니다. S3에서 전체 콘텐츠를 가져와 마크다운으로 렌더링합니다 (remark-gfm, rehype-raw).
3. **기사 상세 페이지**: `/insights/{sourceId}/{docHash}`에서 각 크롤링 문서의 전용 뷰를 제공하며, 전체 콘텐츠, 메타데이터, 관련 기사를 표시합니다.
4. **Research 탭**: Insights 페이지에 세 번째 탭(News | Tech | Research)이 추가되어 Deep Research Agent(별도 ResearchAgentStack)와 연동됩니다. 사용자가 다중 소스 리서치 작업을 시작하고 완료된 보고서를 인라인으로 확인할 수 있습니다.
5. **Claude Sonnet 4.6 요약**: 크롤러 요약이 Haiku에서 Sonnet 4.6으로 업그레이드되어 더 높은 품질의 문서 요약을 제공합니다.
...
- 태그 필터링으로 AWS 서비스별 집중 탐색이 가능합니다 (예: EKS 관련 기사만 표시)
- 인라인 기사 리더로 크롤링 콘텐츠 검토 시 컨텍스트 전환이 줄어듭니다
- Research 탭이 큐레이션된 뉴스/기술 피드와 함께 심층 분석 기능을 제공합니다
- 서버리스 아키텍처로 비용이 낮습니다 (월 ~$12-27)
...
- Sonnet 4.6 요약이 Haiku보다 문서당 비용이 높지만 총 볼륨이 적어 영향은 제한적
...
- `infra/lib/research-agent-stack.ts` -- Deep Research Agent (Bedrock Agent + 도구 Lambda)
- `backend/python/qa/handler.py` -- 현재 `search_aws_docs` 도구가 포함된 QA Lambda
- `backend/python/qa/tools.py` -- KB 검색 필터 로직을 포함한 도구 정의
- `backend/internal/service/insights.go` -- 태그/서비스/소스 필터링이 포함된 ListInsights
- `frontend/src/app/insights/` -- News, Tech, Research 탭이 있는 Insights 페이지
