...
- **Component system**: `MarkdownRenderer`, `Heading`, `Callout`, `CodeBlock`, `MermaidBlock`, `DataTable`, `BlockQuote`, `TOCSidebar`
...
- **Mermaid diagrams**: `mermaid` fenced code blocks rendered via `mermaid` library (lazy loaded). Research Agent generates Mermaid instead of ASCII art for architecture, topology, and flow diagrams.
...
- **Section navigation**: Deep Research reports split by h2 headings into navigable sub-pages. `save_report` tool auto-splits to S3. Section list → section detail with prev/next navigation.
...
주요 구현 사항:
- **컴포넌트 시스템**: `MarkdownRenderer`, `Heading`, `Callout`, `CodeBlock`, `MermaidBlock`, `DataTable`, `BlockQuote`, `TOCSidebar`
- **Callout 문법**: Obsidian 호환 `> [!type] title`을 remark 플러그인으로 파싱
- **코드 강조**: Shiki (WASM 기반, `next/dynamic`으로 지연 로드)
- **Mermaid 다이어그램**: `mermaid` 코드블록을 `mermaid` 라이브러리로 렌더링 (지연 로드). Research Agent가 아키텍처, 토폴로지, 플로우 다이어그램을 ASCII 대신 Mermaid로 생성합니다.
- **TOC**: IntersectionObserver 기반 스크롤 추적이 포함된 우측 고정 사이드바
- **뷰 모드**: 카드(기본) + 테이블 토글 (목록 페이지)
- **섹션 네비게이션**: Deep Research 보고서를 h2 헤딩 기준으로 분할하여 Notion처럼 하위 페이지로 탐색. `save_report` tool이 자동 분할하여 S3 저장. 섹션 목록 → 섹션 상세 + 이전/다음 네비게이션.
