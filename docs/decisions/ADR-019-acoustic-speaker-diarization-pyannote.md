...
- **`meeting.Participants` headcount now flows through** as pyannote's `num_speakers` hint (`backend/cmd/transcribe/main.go` → `NUM_SPEAKERS` env → `transcribe.py`), reducing auto-detection error when the count is known.
...
- **`meeting.Participants` 인원수가 마침내 사용됨**: pyannote의 `num_speakers` 힌트로 전달(`backend/cmd/transcribe/main.go` → `NUM_SPEAKERS` env → `transcribe.py`), 인원수가 알려진 경우 자동 감지 오차를 줄임
