...
- **Account member**: `PK=ACCOUNT#{accountId}`, `SK=MEMBER#{userId}` with `GSI1PK=USER#{userId}`, `GSI1SK=ACCOUNT#{accountId}` for reverse lookup. Roles: `owner` (creator only), `AM`, `TAM`, `SSA`.
...
### Risks
- Roles are coarse (owner vs assignable). Finer per-resource permissions are out of scope for v1.
...
- **Account 멤버**: `PK=ACCOUNT#{accountId}`, `SK=MEMBER#{userId}`, 역방향 조회용 `GSI1PK=USER#{userId}`/`GSI1SK=ACCOUNT#{accountId}`. 역할: `owner`(생성자 전용), `AM`, `TAM`, `SSA`.
...
### 위험
- 역할이 거칠다(owner vs 할당가능). 리소스별 세분 권한은 v1 범위 밖.
