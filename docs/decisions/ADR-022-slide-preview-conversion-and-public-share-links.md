...
- **IAM**: scoped to `docs/*` read + `docs-pdf/*` read/write (`ai-stack.ts`'s `grantRead`/`grantPut` with explicit prefixes) — narrower than the bucket-wide `grantReadWrite` other upload categories (`audio`/`image`/`file`/`doc`) share, because this role additionally runs a third-party document parser (see next point).
...
- **Network isolation**: the Lambda is placed in `PRIVATE_ISOLATED` subnets of the pre-existing VPC (`ec2.Vpc.fromLookup`, the same VPC `WhisperStack` already uses — no new VPC, no NAT cost). `PRIVATE_ISOLATED` has no route to an internet gateway or NAT — a compromised `soffice` process has no network path to exfiltrate credentials over, fetch a document's linked/remote content via SSRF, or reach anything outside this VPC at all. S3 access for the sidecar upload goes out through this shared VPC's **pre-existing** S3 gateway endpoint (`vpce-04a82e15d312f39b8`, already attached to every route table in this VPC, including both `PRIVATE_ISOLATED` subnets' — confirmed via `aws ec2 describe-vpc-endpoints`/`describe-route-tables`). This ADR's first deploy attempt tried to add a *second* S3 gateway endpoint scoped to just those subnets and failed at `cdk deploy` (`AlreadyExists` on the S3 prefix-list route — a route table can only hold one route per prefix list) — do not re-add one; reuse the existing endpoint. This closes the *network* half of the RCE surface. It does **not** close the *IAM* half (see Consequences: the `docs/*` read grant is still cross-tenant) or local-file-read via content embedded directly in the uploaded document itself (no network call needed for that).
...
## References
- **Partially revised by** [ADR-027: CloudFront-Signed Media URLs](ADR-027-cloudfront-signed-media-urls.md) — the sandbox CSP `/media/*` now applies to `docs/*` disables this ADR's "PDF gets the same in-browser preview as converted slides" assumption for direct-upload PDFs specifically (the `docs-pdf/*` conversion sidecar is unaffected).
...
- `backend/cmd/convert-doc/{main.go,Dockerfile}`, `infra/lib/ai-stack.ts` (`convertDocRole`), `infra/lib/gateway-stack.ts` (`convertDocFunction`, `ConvertDocVpc`/`ConvertDocSg` for the `PRIVATE_ISOLATED` placement — no `ConvertDocS3Endpoint`, see below, `DocSlideUploadRule`, `/api/public/docs/{token}` route), `infra/lib/frontend-stack.ts` (`/api/public/*` CloudFront behavior) — implementation.
...
- **Deploy history**: the first `cdk deploy TtobakGatewayStack` attempt for this ADR added a second S3 gateway endpoint (`vpc.addGatewayEndpoint('ConvertDocS3Endpoint', ...)`) scoped to the `PRIVATE_ISOLATED` subnets, on the assumption that a fresh isolated subnet group would need one. It failed (`AlreadyExists` on the S3 prefix-list route — this shared VPC already has an S3 gateway endpoint, `vpce-04a82e15d312f39b8`, attached to every route table including those two) and rolled the whole stack update back. Fixed by removing the redundant `addGatewayEndpoint` call; deployed successfully after. `cdk synth` succeeding does not catch this class of error — it only re-validates against CDK's cached VPC *shape* lookup, which doesn't enumerate existing endpoints; a live `cdk deploy` (or checking `aws ec2 describe-vpc-endpoints` first) is what actually surfaces it.
...
- **IAM**: `docs/*` 읽기 + `docs-pdf/*` 읽기/쓰기로 스코프(`ai-stack.ts`의 명시적 prefix `grantRead`/`grantPut`) — 다른 업로드 카테고리(`audio`/`image`/`file`/`doc`)가 공유하는 버킷 전체 `grantReadWrite`보다 좁습니다. 이 role이 추가로 서드파티 문서 파서를 실행하기 때문입니다(다음 항목 참조).
...
- **네트워크 격리**: 이 Lambda는 기존 VPC(`ec2.Vpc.fromLookup`, `WhisperStack`이 이미 쓰는 그 VPC — 신규 VPC/NAT 비용 없음)의 `PRIVATE_ISOLATED` 서브넷에 배치됩니다. `PRIVATE_ISOLATED`는 인터넷 게이트웨이/NAT로의 라우트가 전혀 없습니다 — 손상된 `soffice` 프로세스가 자격증명을 네트워크로 유출하거나, SSRF로 문서의 링크/원격 콘텐츠를 가져오거나, 이 VPC 밖의 어떤 것에도 접근할 네트워크 경로가 없습니다. 사이드카 업로드용 S3 접근은 이 공유 VPC에 **이미 존재하던** S3 게이트웨이 엔드포인트(`vpce-04a82e15d312f39b8`, 두 `PRIVATE_ISOLATED` 서브넷을 포함해 이 VPC의 모든 라우트 테이블에 이미 연결돼 있음 — `aws ec2 describe-vpc-endpoints`/`describe-route-tables`로 확인됨)를 통해 나갑니다. 이 ADR의 첫 배포 시도는 해당 서브넷에만 스코프한 S3 게이트웨이 엔드포인트를 *추가로* 만들려다 `cdk deploy`에서 실패했습니다(S3 prefix-list 라우트에 `AlreadyExists` — 라우트 테이블은 prefix list당 라우트를 하나만 가질 수 있음) — 다시 추가하지 말고 기존 엔드포인트를 그대로 쓸 것. 이는 RCE 표면의 *네트워크* 절반을 막습니다. *IAM* 절반(결과 참조 — `docs/*` 읽기 권한은 여전히 cross-tenant)이나, 업로드된 문서 자체에 내장된 콘텐츠에 대한 로컬 파일 읽기(네트워크 호출이 필요 없음)는 막지 못합니다.
...
- `backend/cmd/convert-doc/{main.go,Dockerfile}`, `infra/lib/ai-stack.ts`(`convertDocRole`), `infra/lib/gateway-stack.ts`(`convertDocFunction`, `PRIVATE_ISOLATED` 배치를 위한 `ConvertDocVpc`/`ConvertDocSg` — `ConvertDocS3Endpoint`는 없음, 아래 참조 —, `DocSlideUploadRule`, `/api/public/docs/{token}` 라우트), `infra/lib/frontend-stack.ts`(`/api/public/*` CloudFront behavior) — 구현.
- `backend/internal/service/account.go`(`ShareUserDocumentToAccount`, `CreateUserDocPublicShare`, `ResolvePublicShare`), `backend/internal/handler/document.go`(`PublicGetDoc`).
- **배포 이력**: 이 ADR의 첫 `cdk deploy TtobakGatewayStack` 시도는 `PRIVATE_ISOLATED` 서브넷에 스코프한 두 번째 S3 게이트웨이 엔드포인트(`vpc.addGatewayEndpoint('ConvertDocS3Endpoint', ...)`)를 신규 isolated 서브넷 그룹이니 하나 필요할 것이라 가정하고 추가했습니다. 실패했고(S3 prefix-list 라우트에 `AlreadyExists` — 이 공유 VPC에는 이미 S3 게이트웨이 엔드포인트 `vpce-04a82e15d312f39b8`가 두 서브넷을 포함한 모든 라우트 테이블에 연결돼 있었음) 스택 업데이트 전체가 롤백됐습니다. 중복 `addGatewayEndpoint` 호출을 제거해 해결했고, 이후 배포에 성공했습니다. `cdk synth` 성공은 이런 종류의 오류를 잡지 못합니다 — CDK의 캐시된 VPC *형태* lookup만 재검증할 뿐 기존 엔드포인트 목록은 열거하지 않기 때문이며, 실제로 이를 드러내는 것은 실제 `cdk deploy`(또는 사전에 `aws ec2 describe-vpc-endpoints` 확인)뿐입니다.


## 2026-09-12 addendum — source-bound previews

Preview publication records `source-etag` and optional `source-version-id` from the exact downloaded object. The converter checks the current source before publishing and uses destination If-Match/If-None-Match against the preview observed before conversion. Conflicts and uncertain writes fail for the existing Lambda retry flow; retries never delete the prior preview.

The added `docs-pdf/*` read grant permits that initial HEAD. It also expands the accepted cross-tenant read surface to generated previews if LibreOffice is compromised; private-isolated networking and credential stripping remain mitigations, not per-tenant authorization. Source HEAD and destination PUT are not atomic, and ETags do not detect metadata-only changes. Current preview URL generation still checks existence only. Future canonical indexing/retrieval must revalidate source bindings and regenerate legacy unbound previews, never attach current metadata to old bytes. Retry exhaustion leaves the preview unavailable; re-upload can retry conversion.

## 2026-09-12 추가 — 원본 버전과 미리보기 연결

실제 다운로드한 원본의 `source-etag`와 선택적 `source-version-id`를 PDF 메타데이터에 기록합니다. 게시 전 원본을 다시 확인하고, 변환 시작 전 확인한 미리보기에 If-Match/If-None-Match 조건을 적용합니다. 충돌이나 결과가 불명확한 쓰기는 기존 Lambda 재시도로 넘기며 이전 미리보기를 삭제하지 않습니다.

추가된 `docs-pdf/*` 읽기 권한은 최초 HEAD에 필요합니다. LibreOffice가 침해되면 다른 사용자의 생성된 미리보기도 읽을 수 있는 잔여 위험이 추가됩니다. 격리 서브넷과 자격증명 환경변수 제거는 사용자별 권한 경계를 대신하지 않습니다. 원본 HEAD와 결과 PUT은 원자적이지 않고 ETag는 메타데이터만의 변경을 구분하지 못합니다. 현재 미리보기 URL 발급은 존재 여부만 확인하므로, 이후 통합할 색인·검색은 원본 연결 정보를 재검증해야 합니다. 기존 미리보기에는 메타데이터만 덧붙이지 않고 재변환해야 합니다. 재시도가 소진되면 미리보기가 없을 수 있으며 재업로드로 변환을 다시 요청할 수 있습니다.
