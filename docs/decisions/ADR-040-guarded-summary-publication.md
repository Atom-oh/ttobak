...
Batch guards and document collection are active. Saved-summary state, consumer and
API use the same renderer with current edit grants and source checks. Preserve ambiguous immutable spills (ADR-037).
...
정상 주장을 유지합니다. 배치 복구·문서 수집·재요약 API는 활성 코드이며 현재 편집 권한과 소스를 확인합니다.
