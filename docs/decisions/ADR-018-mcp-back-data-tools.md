...
3. **`ttobak_kb_sync` wiring**: the two failure modes differ — `kb.go`'s `SyncKB` returns `{status: "skipped"}` when the `KB_ID`/`KB_DATASOURCE_ID` env vars are missing, but if the env vars are set and the IAM permission is missing, the Bedrock call fails and the endpoint returns HTTP 500. This change adds both halves: `infra/lib/gateway-stack.ts`'s `ApiFunction` now injects `KB_ID`/`KB_DATASOURCE_ID` (note the name landmine — `cmd/api/main.go` reads `KB_DATASOURCE_ID`, *not* the `DATA_SOURCE_ID` name `SummarizeFunction` uses; injecting the wrong name would silently leave sync skipped — a CDK assertion test pins the name), and `apiRole` in `infra/lib/ai-stack.ts` gets `bedrock:StartIngestionJob` scoped to the KB's ARN (narrower than the pre-existing `summarizeRole`/`kbRole`/`crawlerRole` wildcard grants; tightening those three is tracked follow-up). These take effect on the next `TtobakGatewayStack`/`TtobakAiStack` deploy; until then (or on any environment deployed without them) `SyncKB` returns `"skipped"`, and uploads are indexed by other pipelines' ingestion runs on the same data source anyway — `cmd/summarize/main.go` calls `TriggerIngestion` after every completed meeting summary, and the daily crawler triggers one on runs where it added/updated documents (`backend/python/crawler/ingest_trigger.py` skips zero-change runs, and MCP uploads don't count toward its counters). So even in the skipped state an uploaded file becomes searchable at the next meeting-summary completion or document-bearing crawler run. Tool messages/README disclose the skipped-fallback behavior.
...
- Ten tool definitions defined by ADR-018 overall (three in the original decision + seven added post-implementation; the server's full tool list is larger still) to keep in sync across `api.ts`, `index.ts`, and the README's English/Korean tables.

### Risks
- Agents may push low-quality or duplicative documents; only the marker loop-guard exists today, not content-quality checks.
- KB uploads and deletions take effect in search only at the next ingestion run (per Post-Implementation Update 3: `ttobak_kb_sync` once the KB env/IAM wiring from Update 3 is deployed, otherwise a meeting-summary or crawler-triggered run) — a deleted file stays retrievable by its uploader until then.
...
1. **문서 전용을 넘어선 쓰기 표면 확장**: 세 가지 쓰기 가능 도구 그룹이 추가됐다. 각각 여전히 기존의 멤버/소유자 게이트 REST 엔드포인트의 얇은 래퍼(신규 인증 경로 없음):
   ...
   - **`ttobak_create_account`** / **`ttobak_add_account_member`** → `POST /api/accounts`, `POST /api/accounts/{id}/members`. "문서 전용"을 실질적으로 넘어서는 유일한 확장 — 원래 결정이 명시적으로 "미팅·멤버십은 절대 쓰지 않는다"고 했던 부분이 이제 MCP에서 계정·멤버십 쓰기로 확장됐다. 멤버 추가는 여전히 호출자가 이미 계정 소유자여야 하고(서버 강제), role은 할당 가능 집합(`AM`/`TAM`/`SSA`/`SA`/`SA Manager`/`AM Manager`)으로만 제한(`owner`는 불가) — REST 엔드포인트 자체가 강제하는 것과 동일한 제약이다.
...
3. **`ttobak_kb_sync` 배선**: 두 실패 모드가 다르다 — `kb.go`의 `SyncKB`는 `KB_ID`/`KB_DATASOURCE_ID` env가 없으면 `{status: "skipped"}`를 반환하지만, env는 있는데 IAM 권한이 없으면 Bedrock 호출이 실패해 HTTP 500을 반환한다. 이 변경이 두 반쪽을 모두 추가한다: `infra/lib/gateway-stack.ts`의 `ApiFunction`에 `KB_ID`/`KB_DATASOURCE_ID`를 주입하고(이름 함정 주의 — `cmd/api/main.go`는 `KB_DATASOURCE_ID`를 읽으며, `SummarizeFunction`이 쓰는 `DATA_SOURCE_ID`가 *아니다*; 잘못된 이름을 주입하면 sync가 조용히 skipped로 남는다 — CDK assertion 테스트가 이름을 고정), `infra/lib/ai-stack.ts`의 `apiRole`에 해당 KB ARN으로 스코프한 `bedrock:StartIngestionJob`을 부여한다(기존 `summarizeRole`/`kbRole`/`crawlerRole`의 와일드카드 grant보다 좁음; 그 셋의 정리는 후속 추적). 이는 다음 `TtobakGatewayStack`/`TtobakAiStack` 배포 시점에 반영되며, 그 전까지(또는 이들이 없는 환경에서는) `SyncKB`가 `"skipped"`를 반환한다 — 그래도 업로드는 동일 데이터소스에 대한 다른 파이프라인의 인제스천으로 인덱싱된다: `cmd/summarize/main.go`가 미팅 요약 완료 시마다 `TriggerIngestion`을 호출하고, 일일 크롤러도 문서를 추가/갱신한 실행에서 트리거한다(`backend/python/crawler/ingest_trigger.py`는 0건 변경 실행을 skip, MCP 업로드는 그 카운터에 미포함). 즉 skipped 상태에서도 업로드 파일은 다음 미팅 요약 완료 또는 문서를 얻은 크롤러 실행 시점에 검색 가능해진다. skipped-fallback 동작은 도구 메시지/README에 고지한다.
...
- ADR-018 전체가 정의하는 도구가 총 10개(원 결정 3개 + 구현 후 추가분 7개; 서버 전체 도구 수는 그보다 많음) — `api.ts`·`index.ts`·README 영/한 표 간 동기화 대상.

### 위험
- 에이전트가 저품질/중복 문서를 밀어넣을 수 있음; 현재는 마커 루프 가드만 있고 콘텐츠 품질 검사는 없음.
- KB 업로드/삭제는 다음 인제스천 실행 시점에야 검색에 반영됨(구현 후 업데이트 3 참조 — 업데이트 3의 KB env/IAM 배선이 배포되면 `ttobak_kb_sync`, 아니면 미팅 요약/크롤러 트리거) — 삭제한 파일은 그때까지 업로더 본인에게 계속 검색됨.
