...
def _search(user_id, query, count):
    _identifier(user_id)
    ...
    if type(count) is not int or count < 1:
        raise ValueError('Invalid search result limit')
    return {'userId': user_id, 'query': query, 'count': min(count, 10)}
    ...
    try:
        ...
        if kind == 'clientInput':
            ...
            if type(value['contentHash']) is not str or not HEX_REVISION.fullmatch(value['contentHash']):
                return False
        elif set(value) != {'userId', 'query', 'count'} or value != _search(
                value['userId'], value['query'], value['count']):
            return False
...
def request_is_current(user_id, dependency, search, request_meeting_id=None):
    ...
    results = search(value['query'], int(value['count']), user_id=user_id)
...
def remember_empty_search(state, user_id, query, count):
