...
def build_tool_context(user_id, text, source_state, source_details, *, source_access, history,
    ...
    def retrieve(query, count=5, source_keys=None, resource_ids=None, *, source_state, source_details):
        selection = {'source_keys': source_keys} if source_keys is not None else {}
        if resource_ids is not None:
            selection['resource_ids'] = resource_ids
        ...
        if source_keys is not None:
            ...
            found = {result.get('provenance', {}).get('resourceId') for result in results}
            for rid in selected_resource_ids(resource_ids):
                if rid not in found:
                    remember_empty_search(source_state, user_id, query, count, resource_ids=[rid])
        elif not results:
    ...
    context.update({
        'transcript': text,
        'retrieve_from_kb': lambda query, count=5, source_keys=None, resource_ids=None: read_source(
            'search_knowledge_base', retrieve, query, count, source_keys, resource_ids),
