...
| `recording_status`  | `path: string`                                          | `{ recording, temp_path: string \| null, elapsed_ms, finalizing: bool }` — `finalizing` is for `path` specifically, not "is anything finalizing" (polled by RecordButton, passing its own tempPath, after `stop_timed_out` to await background finalize). A current build ALWAYS sends `finalizing`; the TS type marks it optional only for wire-compat with older installed builds, and the SPA fails fast with the version-skew message if it's ever `undefined`. `path` must be under `allowed_dir()` (checked lexically before any FS access) or the answer is `false` without a syscall |
