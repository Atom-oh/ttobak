...
| `recording_status`  | `path: string`                                          | `{ recording, temp_path?, elapsed_ms, finalizing }` — `finalizing` is for `path` specifically, not "is anything finalizing" (polled by RecordButton, passing its own tempPath, after `stop_timed_out` to await background finalize) |
...
- **`run()`'s `.setup()` adopts any leftover `*.wav` under `allowed_dir()` into `recorded_paths` on startup** — a crash or force-quit from a previous run otherwise leaves a file `validate_recording_path` rejects forever ("path was not created by this recording session"), and the `/record?mode=upload` recovery route the error messages point to doesn't go through the native commands at all, so without this the file is permanently un-uploadable/un-deletable through this app. Adoption rejects symlinks (`entry.file_type().is_file()`, checked before `validate_recording_path`'s own containment check ever runs) and deletes (rather than adopts) anything older than `LEFTOVER_RECORDING_MAX_AGE` (48h) — raw meeting audio is PII, and this directory isn't scoped to a Cognito user/session, so unbounded accumulation across every future launch was a real gap, not just theoretical.
