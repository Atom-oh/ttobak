...
- On a Mac, verify with `pmset -g assertions` while recording, after stopping with an upload pending, during a recovered upload, and after cleanup. An older recording's cleanup must leave protection for newer pending recordings. Linux tests cover ownership logic, not IOKit or hardware sleep behavior.
