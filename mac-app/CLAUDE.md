...
- **`capabilities/*.json` needs an explicit `"remote"` grant for the production origin, or every `plugin:event|listen` silently no-ops** (custom commands still work — they bypass the ACL entirely). Tauri 2's ACL only matches a capability's default (local) context against `tauri://localhost`; this window loads `https://ttobak.atomai.click`, so it needs `remote: { urls: [...] }`. This is why `native-audio-level` never reached the UI pre-ADR-024 — nothing looked broken except a waveform that silently never moved. `remote.urls` also lists the CloudFront distribution domain (`d2olomx8td8txt.cloudfront.net`) alongside the custom domain as cheap insurance, even though `tauri.conf.json`'s window is hardcoded to the custom domain today.
