...
src-tauri/
  ...
  src/
    ...
    power.rs          # IOKit idle-sleep assertion + tested ownership across pending recordings
    error.rs          # AppError
```
...
| `release_recording_power` | `path: string` | validates the recorded path and releases only idle-sleep protection; preserves the WAV and whitelist for recovery |
...
- Successful native recording starts acquire a best-effort `PreventUserIdleSystemSleep` IOKit assertion. Pending recordings share it under one mutex, keyed by canonical WAV path; completing one upload cannot release protection for another recording.
- `cleanup_recording` signals upload completion or explicit discard. After validating the path and rejecting active/finalizing recordings, it releases that path's protection before file deletion, so an unlink failure still surfaces without retaining an unnecessary assertion. Failed uploads retain protection while the user can retry. Home/reset, a new draft, page unmount, and terminal native stop errors call `release_recording_power` to release that path without deleting its WAV. Invalid paths or canonicalization failures do not clear pending state.
- Every `upload_recording` attempt additionally holds its own RAII assertion, covering startup-adopted recordings and a concurrent cleanup. It is released when that attempt succeeds, fails, or is cancelled.
- This prevents **idle system sleep only**. Lid-close, Apple-menu sleep, and low-battery sleep can still suspend the Mac; display sleep is allowed. Keep the existing WAV checkpoints and startup recovery. Keep the lid open until upload completes. See [Apple QA1340](https://developer.apple.com/library/archive/qa/qa1340/_index.html).
- On a Mac, verify with `pmset -g assertions` while recording, after stopping with an upload pending, during a recovered upload, and after cleanup or Home/navigation. An older recording's cleanup must leave protection for newer pending recordings. Switching to upload mode while Stop is pending must still deliver the completed WAV to the parent page; if the whole page unmounts, its late file callback releases protection instead. Active native capture remains protected until stop or app exit. Linux tests cover ownership logic, not IOKit or hardware sleep behavior.
